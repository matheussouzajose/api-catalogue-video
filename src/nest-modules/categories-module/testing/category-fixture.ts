...
function createLinks(categoryId: string) {
  return [
    {
      rel: 'self',
      href: `${process.env.API_URL}/categories/${categoryId}`,
      method: 'GET',
    },
    {
      rel: 'update',
    ...
    {
      rel: 'activate',
      href: `${process.env.API_URL}/categories/${categoryId}/activate`,
      method: 'POST',
    },
    {
      rel: 'deactivate',
      href: `${process.env.API_URL}/categories/${categoryId}/deactivate`,
      method: 'POST',
    },
    {
      rel: 'all-categories',
      href: `${process.env.API_URL}/categories?page=1&per_page=10&sort=name&sort_dir=asc&filter=category`,
      method: 'GET',
    },
  ];
}
...
export class CreateCategoryFixture {
  ...
  static arrangeForCreate() {
    const faker = CategoryAggregate.fake()
      .aCategory()
      .withName('Movie')
  ...
  static arrangeForEntityValidationError() {
    const faker = CategoryAggregate.fake().aCategory();
    const defaultExpected = {
      statusCode: 422,
      error: 'Unprocessable Entity',
    };
    ...
    return {
      NAME_TOO_LONG: {
        ...
        expected: {
          message: ['_name must be shorter than or equal to 255 characters'],
          ...defaultExpected,
        },
      },
    };
  ...
  static createLinks(categoryId: string) {
    return createLinks(categoryId);
  }
}
...
export class ListCategoriesFixture {
  static arrangeIncrementedWithCreatedAt() {
    const _entities = CategoryAggregate.fake()
      .theCategories(4)
      .withName((i) => i + '')
